		...
		<view class="forget-bg">
			<view class="forget-card">
				<view class="forget-input forget-margin-b">
					<input type="number" v-model="regInfo.phone" placeholder="请输入您的手机号" />
				</view>
				<view class="forget-input forget-margin-b">
					<view class="verify-left">
						<input type="number" v-model="regInfo.verifyCode" placeholder="请输入验证码" />
					</view>
				...
				<view class="forget-input">
					<input type="number" v-model="regInfo.password" placeholder="请输入密码(6-10位)" />
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			register(){
				...
				var _this=this;
				uni.request({
					url:'http://localhost:8888/codeworld-member/member/register-member',
			...
			sendsms(){
				...
				uni.request({
					url:'http://localhost:8888/codeworld-sms/send-sms?phone=' + this.regInfo.phone,
					method:'POST',
					success: (response) => {
						if(response.data.code === 20000){
							this.$toast (response.data.msg)
