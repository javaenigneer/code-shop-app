	...
	<view class="page">
		...
		<view class="ps-re w100p" :style="'height: '+bannerHeight+';'">
			<swiper class="w100p h100p" circular="true" autoplay="true">
				<swiper-item class="w100p h100p" v-for="(ite,idx) in carouselList" :key="idx" @click="navToBannerPage(ite)">
					<image class="w100p h100p" :src="ite.image"></image>
				</swiper-item>
		...
		<view class="center92 flex" style="margin-top: 0.6rem;height: 9.6rem;">
			...
			<view class="h100p w56p">
				<view class="w100p h50p flex m-center c-center" @click="toaclist(activity1)">
					<view class="w97p h97p brrr2dr">
		...
		<view class="center94 flex m-sb wrap" style="align-content: flex-start; margin-top: 0.2rem;">
			<view class="w48p " style="height:auto; margin-top: 1.5rem;" v-for="(item,index) in goodsList" :key="index" @click="todetail(item)">
				...
				<view class="w100p  fs1r" style="height: 2.5rem;">
					<view class="name h100p" style="font-size: 12px; overflow:hidden;">{{$endwith(item.title,28)}}</view>
				</view>
				<view class="center96  flex m-sb" style="margin-top: 0;">
					<span class="price">￥{{item.promotionPrice==null?item.price:item.promtionPrice}}</span>
...
<script>
	export default {
		...
		methods: {
			...
			// 			location.href=url
			// 		}else{
			// 			this.$dataLocal("page_url",url);
			// 			this.$navigateTo("/pages/common/web-page");

			// 			//restgo.jump_to_exter_page(url)
			// 		}
			// 	}
			...
			getBannerList() {
				uni.request({
					url: 'http://localhost:8888/codeworld-marketing/carouse/get-carouse-enable',
					method: 'GET',
					success: (response => {
						if (response.data.code === 20000) {
							...
							this.carouselList = li;
							this.swiperLength = li.length;
						}
					})
				})
			},
			...
			getProudctList() {
				...
				var that = this;
				uni.request({
					url: 'http://localhost:8888/codeworld-search/goods/get-new-product-index',
					method: 'POST',
					...
					success: (response) => {
						if (response.data.code === 20000) {
							...
							for (var i in li) {
								var item = li[i];
								item.image = that.getLi(item.images)[0] || that.getConst("").default_img;
							}
