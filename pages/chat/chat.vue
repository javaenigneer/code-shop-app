	...
	<view>
		...
		<view class="input-box" :class="showEmji" @touchmove.stop.prevent="discard">
			...
			<view class="more" @tap="chooseImage">
				<view class="iconfont iconicon_photo_fill"></view>
			</view>
			<!-- #endif -->
			<view class="textbox">
		...
		methods:{
			getMsgList(){
				...
				let list = [
					...
					{id:1,uid:1,username:"售后客服008",face:"/static/img/chat/face_2.jpg",time:"12:57",type:"text",msg:{content:"客服连线中，请发送消息等待回复，或者直接拨打客服电话15673341555"}},
				]
				let msg_li=[];
				let local_li=this.$dataLocal("my_msg");
				if(!!local_li&&local_li.length!=0)msg_li=local_li;
				
				list=list.concat(msg_li);
			...
			screenMsg(msg){
				...
				if(!!local_li&&local_li.length!=0)msg_li=local_li;
				msg_li.push(msg);
				this.$dataLocal("my_msg",msg_li);
				
				
				let user=this.getUser();
				let msg_o={};
				msg_o.fromUserId=user.id;
				msg_o.fromUserInfo=JSON.stringify(user);
				
				msg_o.toUserId="99999";
				msg_o.roomId=user.id+"_"+"99999";
				
				let tjk_msg={};
				tjk_msg.type=msg.type;
				tjk_msg.content=msg.msg.content;
				msg_o.msgDetail=JSON.stringify(tjk_msg);
				
				
				this.$post("chat/create",msg_o,res=>{
			...
			sendMsg(content,type){
				...
				let lastid =0;
				let lastmsg=this.msgList[this.msgList.length-1];
				if(this.$notNull(lastmsg))lastid = lastmsg.id;

				lastid++;
				let msg = {id:lastid,uid:0,username:"大黑哥",face:"/static/img/chat/face_1.jpg",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
				this.screenMsg(msg);
				// 定时器模拟对方回复,三秒
				setTimeout(()=>{
					lastid = this.msgList[this.msgList.length-1].id;
					lastid++;
					msg = {id:lastid,uid:1,username:"售后客服008",face:"/static/img/chat/face_2.jpg",time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,msg:content};
					//this.screenMsg(msg);
				},3000)
			},
