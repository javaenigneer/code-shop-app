	...
	<view>
		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content">
				...
				<view class="cen">
					...
					<text class="address">{{addressData.area}}{{addressData.detailed}} {{addressData.houseNumber}}</text>
				</view>
		...
		<view class="goods-section">
			...
			<view class="g-item" v-for="(item, index) in goods_list" :key="index">
				<image :src="item.goods.productImage"></image>
				<view class="right">
					<text class="title clamp">{{item.goods.productTitle}}</text>
					<text class="spec">{{item.sku.title}}</text>
					<view class="price-box">
						<text class="price">￥{{item.sku.price}}</text>
						<text class="number">x {{item.goods.productCount}}</text>
					</view>
	...
	export default {
		...
		methods: {
			...
			locateAddress(_id) {
				...
				this.postage = 0;
				let cityList = this.addressData.area.split("-");
				let prov = cityList[0];
			...
			_submit() {
				...
				var orderInfo = {};
				orderInfo.payTotal = this.goodsTotal;
				...
				for (var i in this.goods_list) {
					...
					var _goods = {};
					_goods.productId = ite.goods.productId;
					_goods.productTitle = ite.goods.productTitle;
					_goods.productImage = ite.goods.productImage;
					_goods.productCount = ite.number;
					// _goods.type=ite.goods.type;
					// _goods.storageNum=ite.goods.storageNum;
					_goods.images = ite.sku.images;
					_goods.id = ite.sku.id;
					_goods.price = ite.sku.price;
					_goods.title = ite.sku.title
					_goods.merchantNumber = ite.goods.merchantNumber
					//    let o={};
					// o.productModel=_goods;
					// o.productSkuModel=ite.sku;
					detail_list.push(_goods);
					cartIds.push(ite.cartId)
					sum_buy_num = sum_buy_num + parseFloat(ite.number);
				}
				// orderInfo.type=_type;
				orderInfo.buyNum = sum_buy_num;
				orderInfo.productInfoSkuModels = JSON.stringify(detail_list);
				orderInfo.cartIds = JSON.stringify(cartIds)
				 
				// orderInfo.goodsMemo=this.desc||this.goods_list[0].goods.name;
				...
				single_user_info.id = this.userInfo.memberId;
				...
				// orderInfo.cusDetail=JSON.stringify(single_user_info);
				orderInfo.memberId = this.userInfo.memberId;
				...
				let _this = this;
				uni.request({
					url: 'http://localhost:8888/codeworld-order/create-order',
					header: {
